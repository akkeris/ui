...
const formSubHeaderStyle = {...GlobalStyles.StandardLabelMargin, ...GlobalStyles.HeaderSmall, ...GlobalStyles.Subtle, fontSize:'0.75rem', textTransform:'uppercase'};
const formTextStyle = {'display':'block', ...GlobalStyles.StandardLabelMargin, ...GlobalStyles.FairlySubtle, ...GlobalStyles.Text};
const formTextEmphasizedStyle = {'display':'block', ...GlobalStyles.StandardLabelMargin, ...GlobalStyles.Text};
...
const stateImage = { maxHeight: '0.9rem', maxWidth: '0.9rem', margin: '0 9px 0 0', verticalAlign: 'middle', flexGrow: '0'}
...
export default class PipelinePromote extends Component {
  ...
  handleOk() {
    this.props.onPromote(this.props.pipeline, this.props.source, this.state.targets, this.state.release, this.state.safe);
  }
  ...
  requiredStatuses() {
    let required = this.state.targets.map((x) => x.required_status_checks ? x.required_status_checks.contexts : []).reduce((acc, cur) => acc.concat(cur), []);
    return required.filter((x, i) => i === required.lastIndexOf(x)); // make unique
  }

  statusByContext(context) {
    let statuses = this.state.release.statuses.statuses;
    let status = statuses.filter((x) => x.context === context);
    if(status.length === 0) {
  ...
  canPromote() {
    let statuses = this.state.release.statuses.statuses;
    let required = this.requiredStatuses();
  ...
  renderSource() {
    let description = `${this.props.source.release.build.commit.message || this.props.source.release.description || ''} ${this.props.source.release.build.author || ''}`;
    let commitUrl = this.props.source.slug.source_blob.url || this.props.source.slug.source_blob.version || '';
    let commitSha = this.props.source.release.build.commit.sha.substring(0, 7);
    return (
        ...
        {!this.state.editRelease ? (
          <div style={{...GlobalStyles.StandardLabelMargin, ...GlobalStyles.NoWrappingText}}>
            ...
            <div style={{...GlobalStyles.StandardLabelMargin, ...GlobalStyles.Subtle, 'marginBottom':'0'}}>
              <label> Deployed <pre style={GlobalStyles.CommitLinkPre}><code>v{this.state.release.version}</code></pre> {util.getDateDiff(this.state.release.created_at)}</label>
            </div>
          </div>
        ) : (
          <div style={{...GlobalStyles.StandardLabelMargin, ...GlobalStyles.NoWrappingText}}>
  ...
  renderTargets() {
    return this.state.targets.map((coupling) => {
      ...
      if(description === '' && coupling.releases.length > 0) {
        description = coupling.releases[coupling.releases.length - 1].description;
      }
      ...
      return (
        <div>
          ...
          {required.map((context, i) => {
            let status = this.statusByContext(context);
            let StatusIcon = statusIcon(status.state);
            let dividerStyle = (i > 0) ? {borderTop:'0px'} : {};
            return (
              <DialogContent key={context} dividers style={{...GlobalStyles.PaperSubtleContainerStyle, ...dividerStyle}}>
  ...
  render() {
    let statusChecksFailed = !this.state.release || !this.state.release.statuses || this.canPromote().length > 0;
    let promoteButtonStyle = !this.state.loading && statusChecksFailed && this.props.isElevated ? GlobalStyles.DangerButton : {};
...
PipelinePromote.propTypes = {
  ...
  couplings: PropTypes.array.isRequired,
